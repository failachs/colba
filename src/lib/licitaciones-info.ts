...
function invalidarToken() { _tokenCache = null; }
...
async function readJson<T>(res: Response, ctx: string): Promise<T> {
  ...
  catch { throw new Error(`[${ctx}] JSON inválido (HTTP ${res.status}): ${raw.slice(0, 200)}`); }
}
...
export async function liciGetTodosProcesos(params: {
  ...
  while (true) {
    ...
    if (page === 1) total = resp.count ?? resp.total ?? lote.length;
    acc.push(...lote);
    if (!lote.length || lote.length < limit || acc.length >= max ||
        (total > 0 && acc.length >= total) || pages >= 200) break;
    page++;
  }

  return { procesos: acc.slice(0, max), paginasConsultadas: pages, totalApi: total };
}

// ── Helper: extraer mejor link de SECOP disponible ───────────────────────────

/**
 * Construye el link directo al portal de origen (SECOP I o II) a partir
 * del codigoProceso y alias_fuente. La API no devuelve el URL de SECOP
 * en campos de fuente — linkSecop/fuentes siempre llegan vacíos.
 *
 * SECOP II: https://community.secop.gov.co/Public/Tendering/OpportunityDetail/Index?noticeUID={codigo}
 * SECOP I:  https://www.contratos.gov.co/consultas/detalleProceso.do?numConstancia={codigo}
 * Fallback: link interno de licitaciones.info
 */
function resolverLinkDetalle(p: LiciProcesoRaw): string {
  ...
  const idContrato = Number(p['idContrato'] ?? 0);

  if (alias === 'S2' || fuente.includes('secop ii') || fuente.includes('secop2')) {
    // SECOP II: busca por código de proceso en el buscador público
    if (codigo) return `https://community.secop.gov.co/Public/Tendering/ContractNoticeManagement/Index?currentNavigation=Agreement&selectedNavigation=Notice&searchText=${encodeURIComponent(codigo)}`;
  }
  if (alias === 'S1' || fuente.includes('secop i') || fuente.includes('secop1')) {
    if (codigo) return `https://www.contratos.gov.co/consultas/detalleProceso.do?numConstancia=${encodeURIComponent(codigo)}`;
  }

  // Fallback: campos directos de fuente si los hubiera
  const directo =
    (p['UrlFuente']    as string | undefined) ||
    (p['url_fuente']   as string | undefined) ||
    (p['LinkFuente']   as string | undefined) ||
    (p['link_fuente']  as string | undefined);
  if (directo?.trim()) return directo.trim();

  if (Array.isArray(p['fuentes'])) {
    for (const f of p['fuentes'] as Array<Record<string, unknown>>) {
      const u = (f['url'] || f['link'] || '') as string;
      if (u?.trim()) return u.trim();
    }
  }

  // Último recurso: link interno licitaciones.info
...
export function normalizarProceso(p: LiciProcesoRaw) {
  ...
  const linkSecop    = String(p['UrlFuente']           ?? p['url_fuente']           ?? p['LinkFuente']           ?? p['link_fuente']           ?? '');
  const linkSecopReg = String(p['UrlFuenteRegistrado'] ?? p['url_fuente_registrado'] ?? p['LinkFuenteRegistrado'] ?? p['link_fuente_registrado'] ?? '');
